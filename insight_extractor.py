...
essays_dir_path = Path('')
questions_dir_path = Path('')
...
# get_completion_from_msg = llm.get_completion_from_msg_with_big_turbo
...
def extractor(question_name, essay_name, index):
    ...
    questions = []
    with open(basic_question_path + question_name, 'r',
              encoding='utf-8') as qfile:
        for line in qfile:
            if not line.strip() == "":
                questions.append(line.strip())

    insight_responses = extract_insight_from_paper_with_questions(
        text, questions)
...
if __name__ == '__main__':
    ...
    for question_name in ['Q1.txt', 'Q3.txt']:
        for index, essay_name in enumerate(essays_name):
